...
const LivePLMonitorPanel: React.FC<LivePLMonitorPanelProps> = ({ plData, historicalData }) => {
      ...
      case 'profit-target':
        return (
          <TrendingUp
            ...
            aria-label="Profit target alert"
          />
        )
      case 'stop-loss':
        return (
        ...
        <div className="space-y-4">
          <div className="text-center">
            ...
            <div className={`text-lg ${getPLColor(plData.unrealizedPLPercent)}`}>
              <strong>
                ({plData.unrealizedPLPercent >= 0 ? '+' : ''}
                {plData.unrealizedPLPercent.toFixed(2)}%)
              </strong>
            </div>
          </div>
          ...
          <div className="relative">
            <div
              ...
              role="progressbar"
              aria-valuenow={Math.abs(plData.unrealizedPLPercent)}
              aria-valuemin={0}
              aria-valuemax={100}
              aria-label={`P/L percentage: ${plData.unrealizedPLPercent.toFixed(2)}%`}
            >
              <motion.div
                className={`h-full ${plData.unrealizedPL >= 0 ? 'bg-green-400' : 'bg-red-400'}`}
                initial={{
                  width: 0,
                }}
                animate={{
          ...
          <div className="flex items-center justify-between p-3 bg-gray-900 rounded">
            ...
            <strong className="text-red-400 font-bold">${plData.timeDecay.toFixed(0)}</strong>
          </div>
        </div>
      </motion.section>
