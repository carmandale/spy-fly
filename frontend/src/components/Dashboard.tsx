...
const Dashboard: React.FC = () => {
    ...
    try {
      ...
      setSentimentData({
        ...
        components: {
          ...
          technical: (data.breakdown.rsi.score + data.breakdown.ma50.score + data.breakdown.bollinger.score) / 3,
  ...
  const fetchOptionsData = async () => {
    try {
      ...
      const calls = data.options.filter((opt: any) => 
        opt.type === 'call' && 
        opt.expiration_date.startsWith(todayExpiration)
      )
