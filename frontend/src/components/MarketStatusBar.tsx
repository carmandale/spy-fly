...
const MarketStatusBar: React.FC<MarketStatusBarProps> = ({
  marketData
}) => {
  ...
  return <motion.header className="bg-gray-800 border border-gray-700 rounded-lg p-4" initial={{
    opacity: 0,
    y: -20
  }} animate={{
    opacity: 1,
    ...
    duration: 0.3
      ...
      <nav className="flex flex-wrap items-center justify-between gap-4 text-sm">
        ...
        <section className="flex items-center space-x-2">
          ...
          <strong className={`font-bold uppercase ${getSessionStatusColor(marketData.sessionStatus)}`}>
