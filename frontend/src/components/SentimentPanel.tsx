...
const SentimentPanel: React.FC<SentimentPanelProps> = ({ sentimentData }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col items-center">
          <div className="relative w-32 h-32 mb-4">
            ...
            <svg
              ...
              viewBox="0 0 120 120"
              role="img"
              aria-label={`Sentiment score: ${sentimentData.overallScore} out of 100`}
            >
              <circle
                cx="60"
                cy="60"
              ...
              <motion.circle
                ...
                fill="none"
                strokeLinecap="round"
                className={getScoreColor(sentimentData.overallScore)}
          ...
          {/* Decision */}
          <motion.div
            className={`text-2xl font-bold ${getDecisionColor(sentimentData.decision)}`}
            initial={{
              scale: 0.8,
            }}
            animate={{
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-4">
            {Object.entries(sentimentData.components).map(([key, value], index) => (
              <motion.li
                key={key}
              ...
              >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="w-16 h-2 bg-gray-700 rounded-full overflow-hidden">
                    <motion.div
                      className={`h-full ${value >= 70 ? 'bg-green-400' : value >= 50 ? 'bg-yellow-400' : 'bg-red-400'}`}
                      initial={{
                        width: 0,
                      }}
                      animate={{
                  ...
                  <strong className={`font-bold text-sm ${getScoreColor(value)}`}>{value}</strong>
                </div>
              </motion.li>
            ))}
          </ul>
