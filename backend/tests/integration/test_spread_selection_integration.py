...
class TestSpreadSelectionIntegration:
    ...
    async def test_full_pipeline_with_realistic_data(self):
        ...
        # Mock option chain
        options = self.create_realistic_option_chain(
            n_strikes=50, spot_price=spot_price
        )
        ...
        recommendations = await self.spread_service.get_recommendations(
            account_size=10000.0, max_recommendations=5
        )
    ...
    async def test_performance_with_large_option_chain(self):
        ...
        options = self.create_realistic_option_chain(
            n_strikes=200, spot_price=spot_price
        )
        mock_chain = OptionChainResponse(
            ticker="SPY",
            underlying_price=spot_price,
        ...
        assert (
            elapsed_time < 10.0
        ), f"Processing took {elapsed_time:.2f}s, expected < 10s"
        assert len(recommendations) > 0
    ...
    async def test_enhanced_spread_selection_methods(self):
        ...
        spot_price = 475.0
        options = self.create_realistic_option_chain(
            n_strikes=30, spot_price=spot_price
        )
        mock_chain = OptionChainResponse(
            ticker="SPY",
            underlying_price=spot_price,
    ...
    async def test_position_sizing_with_small_account(self):
        ...
        # Mock option chain
        options = self.create_realistic_option_chain(
            n_strikes=20, spot_price=spot_price
        )
        mock_chain = OptionChainResponse(
            ticker="SPY",
            underlying_price=spot_price,
        ...
        recommendations = await self.spread_service.get_recommendations(
            account_size=small_account, max_recommendations=3
        )
    ...
    async def test_concurrent_recommendation_requests(self):
        ...
        for account_size in [5000, 10000, 25000]:
            task = self.spread_service.get_recommendations(
                account_size=account_size, max_recommendations=3
            )
            tasks.append(task)
