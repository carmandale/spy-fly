...
from app.models.market import OptionChain, OptionContract, Quote
...
class TestMarketDataService:
    ...
    def service(self, mock_polygon_client, cache, rate_limiter):
        return MarketDataService(
            polygon_client=mock_polygon_client, cache=cache, rate_limiter=rate_limiter
        )
    ...
    async def test_option_filtering(self, service, mock_polygon_client):
        ...
        result = await service.get_spy_options(
            expiration=date(2025, 7, 26), option_type="call", strike_range=5
        )
        ...
        result_all = await service.get_spy_options(
            expiration=date(2025, 7, 26), option_type="call", strike_range=10
        )
