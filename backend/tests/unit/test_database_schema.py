...
class TestDatabaseSchema:
    ...
    def test_market_data_cache_schema(self, test_engine):
        ...
        columns = {col['name']: col for col in inspector.get_columns('market_data_cache')}
    ...
    def test_option_contracts_schema(self, test_engine):
        ...
        columns = {col['name']: col for col in inspector.get_columns('option_contracts')}
        
        # Check all columns exist
        expected_columns = [
            'id', 'symbol', 'underlying', 'strike', 'option_type',
            'expiration_date', 'bid', 'ask', 'last_price', 'volume',
            'open_interest', 'timestamp', 'created_at'
        ]
    ...
    def test_historical_prices_schema(self, test_engine):
        ...
        columns = {col['name']: col for col in inspector.get_columns('historical_prices')}
        
        # Check OHLCV columns
        ohlcv_columns = ['open_price', 'high_price', 'low_price', 'close_price', 'volume']
...
class TestDatabaseModels:
    ...
    def test_market_data_cache_crud(self, test_session: Session):
        ...
        # Read
        retrieved = test_session.query(MarketDataCache).filter_by(
            cache_key="SPY_QUOTE_2025-07-27"
        ).first()
        ...
        # Update
        retrieved.expires_at = datetime.now(timezone.utc) + timedelta(minutes=10)
        test_session.commit()
    ...
    def test_spy_quote_crud(self, test_session: Session):
        ...
        # Read
        latest_quote = test_session.query(SPYQuote).order_by(
            SPYQuote.timestamp.desc()
        ).first()
    ...
    def test_option_contract_crud(self, test_session: Session):
        ...
        zero_dte = test_session.query(OptionContract).filter(
    ...
    def test_historical_price_crud(self, test_session: Session):
        ...
        daily_prices = test_session.query(HistoricalPrice).filter(
            HistoricalPrice.timeframe == "daily",
    ...
    def test_api_request_log_crud(self, test_session: Session):
        ...
        # Read - find errors
        errors = test_session.query(APIRequestLog).filter(
