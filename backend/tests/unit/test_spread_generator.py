...
from app.services.spread_generator import SpreadCombination, SpreadGenerator
...
class TestSpreadGenerator:
    ...
    def test_vectorized_generation_performance(self):
        ...
        assert (
            generation_time < 1.0
        ), f"Generation took {generation_time:.2f}s, expected < 1s"
    ...
    def test_spread_width_filtering(self):
        ...
        filtered = self.generator.filter_by_width(
            spreads, min_width=5.0, max_width=10.0
        )
    ...
    def test_liquidity_based_spread_filtering(self):
        ...
        filtered = self.generator.filter_by_liquidity(
            spreads, self.sample_options, min_liquidity_score=60
        )
    ...
    def test_position_sizing_calculation(self):
        ...
        spreads = self.generator.generate_spreads(
            self.sample_options[:2]
        )  # Just 1 spread
        spread = spreads[0]
        ...
        position = self.generator.calculate_position_size(
            spread, account_size=account_size, max_risk_pct=max_risk_pct
        )
    ...
    def test_bulk_spread_generation(self):
        ...
        spreads = self.generator.generate_filtered_spreads(
            self.sample_options, config=config
        )
    ...
    def test_spread_sorting_by_score(self):
        ...
        sorted_by_rr = self.generator.sort_spreads(
            spreads, sort_by="risk_reward_ratio", descending=True
        )
        ...
        sorted_by_cost = self.generator.sort_spreads(
            spreads, sort_by="net_debit", descending=False
        )
