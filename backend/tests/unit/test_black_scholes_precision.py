...
class TestBlackScholesPrecision:
    ...
    def test_option_price_put_call_parity(self):
        ...
        assert (
            synthetic_put > 0
        ), f"Synthetic put price {synthetic_put} should be positive"
    ...
    def test_probability_mathematical_bounds(self):
        ...
        for S, K, T, sigma, description in test_scenarios:
            prob = self.calculator.probability_of_profit(
                spot_price=S, strike_price=K, time_to_expiry=T, volatility=sigma
            )
    ...
    def test_volatility_sensitivity_accuracy(self):
        ...
        for vol in volatilities:
            prob = self.calculator.probability_of_profit(volatility=vol, **base_params)
    ...
    def test_time_decay_mathematical_accuracy(self):
        ...
        for time in times:
            ...
            price = self.calculator.option_price(time_to_expiry=time, **base_params)
            probabilities.append(prob)
    ...
    def test_reference_implementation_comparison(self):
        ...
        assert (
            abs(d1_ours - d1_manual) < 1e-15
        ), f"d1 mismatch: ours={d1_ours:.15f}, manual={d1_manual:.15f}"
        assert (
            abs(d2_ours - d2_manual) < 1e-15
        ...
        assert (
            abs(prob_ours - prob_manual) < 1e-15
    ...
    def test_numerical_stability_extreme_values(self):
        ...
        assert (
            prob_deep_itm > 0.99
        ), f"Deep ITM probability {prob_deep_itm} should be > 99%"
