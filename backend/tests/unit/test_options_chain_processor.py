...
from app.models.market import OptionContract, OptionChainResponse
...
class TestOptionsChainProcessor:
	...
	def test_parse_polygon_option_chain(self):
		...
		assert all(col in df.columns for col in [
			'strike', 'bid', 'ask', 'mid', 'volume', 'open_interest', 'bid_ask_spread'
		])
	...
	def test_calculate_liquidity_metrics(self):
		...
		options = [
			self.create_mock_option_contract(475.0, 3.00, 3.10, volume=1000, open_interest=500),
			self.create_mock_option_contract(480.0, 1.50, 1.70, volume=50, open_interest=20),
			self.create_mock_option_contract(485.0, 0.50, 0.80, volume=5, open_interest=2),
		]
		...
		assert df.loc[0, 'bid_ask_spread_pct'] == pytest.approx((0.10 / 3.05) * 100, abs=0.01)
	...
	@pytest.mark.parametrize("volume,oi,expected_liquid", [
