...
from app.models.market import Quote, OptionContract, Bar
...
class TestPolygonClient:
    ...
    async def test_get_quote_not_found(self, client):
        ...
        with patch.object(client, '_make_request', side_effect=PolygonAPIError("Ticker not found")):
    ...
    async def test_rate_limit_handling(self, client):
        mock_response = Mock(
            ...
            raise_for_status=Mock(side_effect=httpx.HTTPStatusError("Rate limit", request=Mock(), response=Mock()))
        )
    ...
    async def test_authentication_error(self, client):
        mock_response = Mock(
            status_code=403,
            raise_for_status=Mock(side_effect=httpx.HTTPStatusError("Forbidden", request=Mock(), response=Mock(status_code=403)))
        )
...
class TestPolygonDataService:
    ...
    async def test_historical_data_with_date_range(self, data_service, polygon_client):
        ...
        async def mock_get_historical_data(days=30):
            ...
            return await polygon_client.get_historical_bars("SPY", from_date, to_date, "day")
