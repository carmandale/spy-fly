...
class TestRiskValidator:
    ...
    def test_validate_buying_power_within_limit(self):
        ...
        recommendation = self.create_test_recommendation(
            net_debit=2.00, contracts=2, account_size=10000.0
        )

        result = self.validator.validate_buying_power(
            recommendation, account_size=10000.0
        )
    ...
    def test_validate_buying_power_exceeds_limit(self):
        ...
        recommendation = self.create_test_recommendation(
            net_debit=4.00, contracts=2, account_size=10000.0
        )
    ...
    def test_validate_buying_power_edge_cases(self):
        ...
        recommendation = self.create_test_recommendation(
            net_debit=2.50, contracts=2, account_size=10000.0
        )
        result = self.validator.validate_buying_power(
            recommendation, account_size=10000.0
        )
    ...
    def test_validate_risk_reward_ratio_valid(self):
        ...
        recommendation = self.create_test_recommendation(
            net_debit=2.50, max_profit=2.50
        )
        ...
        recommendation = self.create_test_recommendation(
            net_debit=2.00, max_profit=4.00
        )
    ...
    def test_validate_risk_reward_ratio_invalid(self):
        ...
        recommendation = self.create_test_recommendation(
            net_debit=4.00, max_profit=2.00
        )
    ...
    def test_validate_risk_reward_ratio_edge_cases(self):
        ...
        recommendation = self.create_test_recommendation(net_debit=2.50, max_profit=0.0)
        ...
        recommendation = self.create_test_recommendation(net_debit=0.0, max_profit=2.50)
    ...
    def test_calculate_position_size_edge_cases(self):
        ...
        with pytest.raises(ValueError, match="Net debit must be positive"):
            self.validator.calculate_position_size(
                account_size=10000.0, net_debit=0.0, max_buying_power_pct=0.05
            )

        # Negative net debit
        with pytest.raises(ValueError, match="Net debit must be positive"):
            self.validator.calculate_position_size(
                account_size=10000.0, net_debit=-2.50, max_buying_power_pct=0.05
            )

        # Zero buying power percentage
        with pytest.raises(
            ValueError, match="Buying power percentage must be positive"
        ):
            self.validator.calculate_position_size(
                account_size=10000.0, net_debit=2.50, max_buying_power_pct=0.0
            )
    ...
    def test_validate_multiple_spreads_batch(self):
        ...
        spreads = [
            self.create_test_recommendation(
                net_debit=2.00, max_profit=3.00, contracts=4, account_size=account_size
            ),  # Valid - 4%
            self.create_test_recommendation(
                net_debit=5.00, max_profit=2.00, contracts=2, account_size=account_size
            ),  # Invalid R/R - 0.4:1
            self.create_test_recommendation(
                net_debit=3.00, max_profit=3.00, contracts=10, account_size=account_size
            ),  # Invalid BP - 15%
            self.create_test_recommendation(
                net_debit=1.50, max_profit=2.00, contracts=3, account_size=account_size
            ),  # Valid - 2.25%
        ]
        ...
        assert (
            results["validation_details"][1]["errors"][0]
    ...
    def test_position_size_with_custom_limit(self):
        ...
        result = validator.calculate_position_size(
            account_size=account_size, net_debit=net_debit, max_buying_power_pct=0.03
        )
    ...
    def test_risk_validator_configuration(self):
        ...
        custom_validator = RiskValidator(
            max_buying_power_pct=0.02, min_risk_reward_ratio=1.5
        )
        ...
        self.validator.update_configuration(
            max_buying_power_pct=0.04, min_risk_reward_ratio=1.2
        )
