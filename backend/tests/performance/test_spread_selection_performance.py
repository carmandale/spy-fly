...
class TestSpreadSelectionPerformance:
    ...
    def create_large_option_chain(self, n_strikes=500, spot_price=475.0):
        ...
        if n_strikes <= 100:
            ...
            strikes = np.arange(spot_price - strike_range, spot_price + strike_range, 1.0)[:n_strikes]
        else:
    ...
    async def test_performance_500_strikes(self):
        ...
        assert elapsed_time < 10.0, f"Processing 500 strikes took {elapsed_time:.2f}s, expected < 10s"
        ...
        print(f"- Spread combinations evaluated: ~{len(options) * (len(options) - 1) // 2}")
    ...
    async def test_component_performance_breakdown(self):
        ...
        # Create test data
        options = self.create_large_option_chain(n_strikes=n_strikes, spot_price=spot_price)
        mock_chain = OptionChainResponse(
            ticker="SPY",
            underlying_price=spot_price,
            expiration=date.today().isoformat(),
        ...
        print(f"- Options processing: {processing_time:.3f}s ({processing_time/total_time*100:.1f}%)")
        print(f"- Spread generation: {generation_time:.3f}s ({generation_time/total_time*100:.1f}%)")
        print(f"- Spread filtering: {filtering_time:.3f}s ({filtering_time/total_time*100:.1f}%)")
        print(f"- Probability calc: {prob_calc_time:.3f}s ({prob_calc_time/total_time*100:.1f}%)")
    ...
    def test_vectorization_performance_comparison(self):
        ...
        assert speedup > 2.0, f"Vectorization speedup only {speedup:.1f}x, expected > 2x"
        assert len(spreads_normal) == len(spreads_vectorized), "Different number of spreads generated"