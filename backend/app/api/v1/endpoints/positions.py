...
from app.models.position import PLAlert, Position, PositionPLSnapshot
...
class PositionCreate(BaseModel):
    ...
    entry_vix: float | None = Field(None, description="VIX level at entry")
    entry_sentiment_score: float | None = Field(None, description="Sentiment score at entry")
...
class PositionUpdate(BaseModel):
    ...
    profit_target_percent: float | None = Field(None, description="Updated profit target %")
    stop_loss_percent: float | None = Field(None, description="Updated stop loss %")
...
class PositionResponse(BaseModel):
    ...
    exit_long_premium: float | None
    exit_short_premium: float | None
    exit_net_credit: float | None
    exit_total_value: float | None
...
class PLSnapshotResponse(BaseModel):
    ...
    # Current pricing
    current_long_premium: float | None
    current_short_premium: float | None
    current_net_value: float | None
    ...
    position_gamma: float | None
    position_theta: float | None
    position_vega: float | None

    # Time decay
    time_to_expiry_hours: float | None
    daily_theta_decay: float | None
