...
class RiskValidationResult:
    ...
    max_allowed_percentage: float | None = None
    actual_ratio: float | None = None
    min_required_ratio: float | None = None
...
class RiskValidator:
    ...
    def __init__(
        self, max_buying_power_pct: float = 0.05, min_risk_reward_ratio: float = 1.0
    ):
        """
    ...
    def validate_buying_power(
        self, recommendation: SpreadRecommendation, account_size: float
    ...
    def validate_risk_reward_ratio(
        self, recommendation: SpreadRecommendation
    ) -> RiskValidationResult:
        """
    ...
    def calculate_position_size(
        ...
        if max_buying_power_pct <= 0:
            raise ValueError(
                f"Buying power percentage must be positive, got {max_buying_power_pct}"
            )
    ...
    def validate_spreads_batch(
        self, recommendations: list[SpreadRecommendation], account_size: float
    ) -> dict[str, Any]:
        """
