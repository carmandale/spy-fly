...
class SpreadGenerator:
    ...
    def generate_spreads(
        self, options_df: pd.DataFrame, skip_invalid: bool = True
        ...
        for i in range(len(options_df)):
            ...
            for j in range(i + 1, len(options_df)):
                ...
                if skip_invalid and (
                    short_option["bid"] <= 0 or short_option["ask"] <= 0
                ):
                    continue
    ...
    def generate_spreads_vectorized(
        ...
        risk_reward_ratios = np.divide(
            max_profits, max_risks, out=np.zeros_like(max_profits), where=max_risks > 0
        )
    ...
    def filter_by_risk_reward(
        self, spreads: list[SpreadCombination], min_ratio: float = 1.0
    ) -> list[SpreadCombination]:
        """
    ...
    def filter_by_liquidity(
        ...
        for spread in spreads:
            ...
            if (
                spread.long_liquidity_score < min_liquidity_score
                or spread.short_liquidity_score < min_liquidity_score
            ):
                continue
    ...
    def calculate_position_size(
        self, spread: SpreadCombination, account_size: float, max_risk_pct: float = 0.05
    ) -> dict[str, Any]:
        """
    ...
    def sort_spreads(
        ...
        """
        return sorted(spreads, key=lambda s: getattr(s, sort_by), reverse=descending)

    def generate_filtered_spreads(
        self, options_df: pd.DataFrame, config: dict[str, Any] | None = None
    ) -> list[SpreadCombination]:
        """
        ...
        spreads = self.filter_by_risk_reward(
            spreads, min_ratio=default_config["min_risk_reward_ratio"]
        )
        ...
        spreads = self.sort_spreads(
            spreads, sort_by=default_config["sort_by"], descending=True
        )

        return spreads

    def _create_spread_combination(
        self, long_option: pd.Series, short_option: pd.Series
    ) -> SpreadCombination:
