...
def score_vix(vix_value: float | None) -> ComponentScore:
    """Score VIX level."""
    if vix_value is None:
        return ComponentScore(
            score=0, value=0, threshold="N/A", label="VIX data unavailable"
        )
    ...
    return ComponentScore(
        score=score, value=vix_value, threshold=threshold, label=label
    )


def score_futures(
    current: float | None, previous_close: float | None
) -> ComponentScore:
    """Score S&P 500 futures movement."""
    if current is None or previous_close is None:
        return ComponentScore(
            score=0, value=0, change_percent=0, label="Futures data unavailable"
        )
    ...
    return ComponentScore(
        score=score, value=current, change_percent=round(change_percent, 3), label=label
    )
...
def score_rsi(rsi_value: float | None) -> ComponentScore:
    """Score RSI indicator."""
    if rsi_value is None:
        return ComponentScore(score=0, value=50, range="N/A", label="RSI unavailable")
...
def score_ma50(current_price: float, ma50: float) -> ComponentScore:
    ...
    return ComponentScore(
        score=score, value=current_price, position=position, label=label
    )
...
def score_bollinger(position: float | None) -> ComponentScore:
    """Score Bollinger band position."""
    if position is None:
        return ComponentScore(score=0, value=0.5, label="Bollinger bands unavailable")

    # Score 10 if in middle 60% (0.2 to 0.8)
    if (
        settings.bollinger_inner_range / 2
        <= position
        <= 1 - (settings.bollinger_inner_range / 2)
    ):
        score = 10
    ...
    return ComponentScore(score=score, value=round(position, 3), label=label)


def score_news() -> ComponentScore:
    """Placeholder for news sentiment scoring."""
    # TODO: Implement actual news sentiment analysis
    return ComponentScore(score=15, value=0, label="Neutral market sentiment")
...
class SentimentCalculator:
    ...
    async def calculate_sentiment(self, force_refresh: bool = False) -> SentimentResult:
        ...
        cache_key = self.cache.generate_key(
            "sentiment", "SPY", date=datetime.now().date().isoformat()
        )
        ...
        try:
            ...
            bollinger_position = (
                calculate_bollinger_position(prices) if len(prices) >= 20 else None
            )

            # Score each component
            vix_score = score_vix(vix_data.get("value"))
            futures_score = score_futures(
                futures_data.get("current"), futures_data.get("previous_close")
            )
            rsi_score = score_rsi(rsi_value)
            ma50_score = (
                score_ma50(prices.iloc[-1], ma50_value)
                if ma50_value
                else ComponentScore(
                    score=0,
                    value=0,
                    label=f"Insufficient data for MA (need 20+ days, have {len(prices)})",
                )
            )
            ...
            technicals_bullish = (
                ma50_score.score > 0  # Above MA50
                and rsi_score.score > 0  # RSI in healthy range
                and bollinger_score.score > 0  # Not at extremes
            )
            ...
            decision = (
                "PROCEED"
                if (
                    total_score >= settings.sentiment_minimum_score
            ...
            # Cache the result
            self.cache.set(
                cache_key, result.model_dump(), ttl=settings.sentiment_cache_ttl
            )
    ...
    async def _get_futures_data(self) -> dict[str, Any]:
        ...
        # For now, return mock data
        return {"current": 5680.0, "previous_close": 5670.0}
