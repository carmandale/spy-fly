...
class MarketDataService:
    ...
    async def get_spy_quote(self) -> QuoteResponse:
        ...
        try:
            ...
            # Cache the response
            self.cache.set(
                cache_key, response.model_dump(), ttl=settings.polygon_cache_ttl_quote
            )
    ...
    async def get_spy_options(
        ...
        try:
            ...
            if option_type:
                filtered_options = [
                    opt for opt in filtered_options if opt.type == option_type
                ]
            ...
            # Cache the response
            self.cache.set(
                cache_key, response.model_dump(), ttl=settings.polygon_cache_ttl_options
            )
    ...
    async def get_historical_data(
        self, days: int, timeframe: str = "day"
    ) -> HistoricalDataResponse:
        ...
        try:
            ...
            bars = await self.polygon.get_historical_bars(
                "SPY", from_date, to_date, timeframe
            )
