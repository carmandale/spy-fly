...
class OptionsChainProcessor:
	...
	def add_liquidity_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
		...
		spread_score = np.clip(1 - (df['bid_ask_spread_pct'] / 10), 0, 1) * 30  # 30% weight
	...
	def filter_by_liquidity(
		...
		if require_both:
			liquidity_mask = (df['volume'] >= min_volume) & (df['open_interest'] >= min_open_interest)
		else:
			liquidity_mask = (df['volume'] >= min_volume) | (df['open_interest'] >= min_open_interest)
	...
	def _create_empty_dataframe(self) -> pd.DataFrame:
		...
		return pd.DataFrame(columns=[
			'symbol', 'strike', 'expiration', 'bid', 'ask', 'mid',
			'last', 'volume', 'open_interest', 'bid_ask_spread'
		])