...
class Settings(BaseSettings):
    ...
    def validate_startup_configuration(self) -> tuple[bool, list[str]]:
        ...
        if not self.polygon_api_key:
            errors.append("POLYGON_API_KEY is not configured. Get your free API key at https://polygon.io/")
        ...
        if self.vix_high_threshold <= self.vix_low_threshold:
            errors.append(f"VIX_HIGH_THRESHOLD ({self.vix_high_threshold}) must be greater than VIX_LOW_THRESHOLD ({self.vix_low_threshold})")
        
        # Check RSI thresholds make sense
        if self.rsi_overbought <= self.rsi_oversold:
            errors.append(f"RSI_OVERBOUGHT ({self.rsi_overbought}) must be greater than RSI_OVERSOLD ({self.rsi_oversold})")
